...
class Api::SessionsController < ApplicationController
  ...
  def login
    ...
    if user && user.authenticate(params[:password])
      token = Auth.create_token({ id: user.id, email: user.email, coldSensitivity: user.cold_sensitivity, optsHandsFree: user.opts_hands_free, userCities: user.user_cities, cities: user.cities })
